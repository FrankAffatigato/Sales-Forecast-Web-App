...
def load_data():
    df = pd.read_csv("enhanced_sample_sales_large.csv")
...
col1.markdown("<div class='metric-box'><p class='big-font'>🛒 Total Sales</p><h3 style='color:#000'>${:,.0f}</h3></div>".format(total_sales), unsafe_allow_html=True)
...
col4.markdown("<div class='metric-box'><p class='big-font'>📉 Avg. Daily Sales</p><h3 style='color:#000'>${:,.0f}</h3></div>".format(avg_sales), unsafe_allow_html=True)
...
# ---------- VISUAL: Store Cluster with Full Features ----------
st.markdown("### 🧭 Clustered Store Performance")
store_metrics = raw_df.groupby("store_id").agg({
    "sales": "mean",
    "margin": "mean",
    "out_of_stock_pct": "mean"
}).reset_index()
scaler = StandardScaler()
X_scaled = scaler.fit_transform(store_metrics[["sales", "margin", "out_of_stock_pct"]])
kmeans = KMeans(n_clusters=4, random_state=42).fit(X_scaled)
store_metrics["Cluster"] = kmeans.labels_
fig_cluster_store = px.scatter(
    store_metrics,
    x="sales",
    y="margin",
    size="out_of_stock_pct",
    color="Cluster",
    hover_name="store_id",
    title="Store Clustering Based on KPIs"
)
fig_cluster_store.update_traces(marker=dict(sizemode='diameter', sizeref=0.5, sizemin=10))
fig_cluster_store.update_layout(template="plotly_white", height=500)
st.plotly_chart(fig_cluster_store, use_container_width=True)

# ---------- VISUAL: Product Cluster with Full Features ----------
st.markdown("### 🧪 Clustered Product Performance")
product_metrics = raw_df.groupby("product_id").agg({
    "sales": "mean",
    "margin": "mean",
    "out_of_stock_pct": "mean"
}).reset_index()
X_prod_scaled = scaler.fit_transform(product_metrics[["sales", "margin", "out_of_stock_pct"]])
kmeans_prod = KMeans(n_clusters=4, random_state=42).fit(X_prod_scaled)
product_metrics["Cluster"] = kmeans_prod.labels_
fig_cluster_product = px.scatter(
    product_metrics,
    x="sales",
    y="margin",
    size="out_of_stock_pct",
    ...
    text="product_id",
    title="Product Clustering Based on KPIs"
)
fig_cluster_product.update_traces(textposition='top center', marker=dict(sizemode='diameter', sizeref=0.5, sizemin=10))
fig_cluster_product.update_layout(template="plotly_white", height=500)
st.plotly_chart(fig_cluster_product, use_container_width=True)
